...
```{r 4-my-eyez-only, include = FALSE}
# I'll be using these variables in the text! Sorry :)
borough <- entire.tb %>% select(boro) %>% unique
name <- entire.tb %>% select(dba) %>% unique
```

(All above records belong to `r borough` restaurant *`r name`*)
...
```{r sanity-checking, include = FALSE, eval = FALSE}
# Just checking -- please ignore!
random.camis <- first.last.grading %>%
  filter(grade.first == "Z" & grade.last == "Z") %>%
  get.random.camis

violations %>%
  filter(camis == random.camis) %>%
  data.frame
```
